// ============================================================================
// MODUL 05B — Smart Sensor Pintu (Node B)
//
// Peran : BLE peripheral, dipasang di PINTU 1.
// Sensor: tombol BOOT (GPIO9) mensimulasikan proximity/reed switch pintu.
//         Ditekan  -> pintu dianggap TERBUKA
//         Dilepas  -> pintu dianggap TERTUTUP
...
#define SENSOR_ID "PINTU1"
#define BLE_NAME "SENSOR_PINTU1"
...
void setup() {
  Serial.begin(115200);
  Serial.println("Smart sensor PINTU 1 (peripheral) starting...");
  ...
  Serial.println("Menunggu hub... (tekan tombol BOOT = pintu dibuka)");
}
...
void loop() {
  ...
  static bool lastStable = HIGH;      // HIGH = tidak ditekan = pintu tertutup
  ...
  lastStable = raw;
  bool isOpen = (raw == LOW);  // tombol ditekan -> LOW -> pintu terbuka
  ++eventCount;
